...
class GaborCNN:
  ...
  def construct(self, recurrents=1):
    y = self.x
    outChan = None
    outWidth = None
    outHeight = None
    ...
    # convolution
    convSize = ssti/16+1;
    poolSize = ssti - convSize + 1;
    convKernel = gabor_kernel(convSize, 1, self.nneurons)
    self.timeElapsed("gabor_kernel ok")
    convBias = bias_variable([self.nneurons])
    ...
    # here y's shape should be [batch, 1, 1, nneurons]
